...
curl_easy_cleanup(easyhandle); // erase the html from
curl_object/buffer
fclose(file);
}
...
icase means not case sensitive
vector<string> links; // it will store all the
links read from file
// below line will store all found links in vector
copy(sregex_token_iterator(html.begin(), html.end(), hl_regex, 1),
...
cleaned_link.push_back(one_link[j]); // concatenating character by
character to string
}
// after getting the useful part now validate the link, if the URL is good to
go then add it to the final_vector
if (regex_match(cleaned_link,
regex("((http|https)://)(www.)?[a-zA-Z0-9@:%._\\+~#?&//=]{2,256}\\.[a-z]{2,6
}\\b([-a-zA-Z0-9@:%._\\+~#?&//=]*)")))
{
...
unordered_map<string, bool> visited; // used while dfs to identify url is already
visited or not + Insertion TC = O(1)
...
visited[s] = 1; // mark the current URL as visited
m.lock(); // using it for synchronization
cout << "Bot_id: " << bot_id << "\tLink: " << url << endl; // print the URL
and thread id
get_page(url, file_path); // get HTML of the current
URL
...
thread t1(dfs_crawler, "https://www.tofugu.com/learn-japanese/", filename, 0,
1);
